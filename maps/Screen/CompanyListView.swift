...
struct CompanyListView: View {
    ...
    let companies: [CompanyModel] = Bundle.main.decode("companies.json")
    ...
    var itemsList = ["groceries": ["Grocery"],"fastfood": ["Fast foods"],"pharmacy": ["Pharmacy"],"autoshops": ["Autoshops"]] // Flip with categoriesList
                    ...
                    LazyVGrid(columns: columns, spacing: 20) {
                        ForEach(companies) { item in
                            if self.searchField == "" {
                                ...
                                if item.name.lowercased().contains(self.searchField.lowercased()){
                                    ZStack {
                                        Button {
                                            self.cordinates = item.location
                                            self.showItemView = true
...
struct CompanyListView_Previews: PreviewProvider {
    static var previews: some View {
        CompanyListView(categorytype: .constant(String()))
    }
}
